...
public class FeedbackTest extends BaseClass {
	...
	@Test
	public void sendFeedback() throws Throwable {
		...
		home.getSendFeedback().click();
		Assert.assertTrue(feedback.getSendFeedbackText().getText().equals(eUtils.readDataFromExcel("Feedback", 0, 1)),
				"Feedback page is not displayed");
		feedback.getFeedbackTextField().sendKeys(eUtils.readDataFromExcel("Feedback", 1, 1));
		feedback.getSendButton().click();
		webUtils.explicitWaitForVisibility(wait, favourites.getToastElement());
		Assert.assertTrue(favourites.getToastElement().getText().equals(eUtils.readDataFromExcel("Feedback", 2, 1)),
				"The toast message is not received");
	}
}
